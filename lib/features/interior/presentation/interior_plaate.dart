...
class InteriorColorPaletteScreen extends StatefulWidget {
  const InteriorColorPaletteScreen({super.key});

  static const routeName = "/interior-color-palette-screen";

  @override
  State<InteriorColorPaletteScreen> createState() =>
      _InteriorColorPaletteScreenState();
}

class _InteriorColorPaletteScreenState
    extends State<InteriorColorPaletteScreen> {
  ...
  Widget _buildSmartIcon() {
    final containerSize = r.adaptiveValue(context, mobile: 42, tablet: 52);
    final iconSize = r.adaptiveValue(context, mobile: 24, tablet: 32);
    return Container(
      width: containerSize,
  ...
  Widget _buildSwatches(List<Color> colors) {
    ...
    final double size = r.adaptiveValue(context, mobile: 34, tablet: 44);
    final double overlap = r.adaptiveValue(context, mobile: 10, tablet: 14);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        color: color,
        border: Border.all(color: Colors.white, width: r.wp(context, 1.8)),
        boxShadow: [
          BoxShadow(
...
class _CubePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final sw = size.width / 24.0;
    final sh = size.height / 24.0;

    // Top face – purple/pink
    final topPath =
        Path()
          ..moveTo(cx, cy - 8 * sh)
          ..lineTo(cx + 9 * sw, cy - 3 * sh)
          ..lineTo(cx, cy + 2 * sh)
          ..lineTo(cx - 9 * sw, cy - 3 * sh)
          ..close();
    ...
    final leftPath =
        Path()
          ..moveTo(cx - 9 * sw, cy - 3 * sh)
          ..lineTo(cx, cy + 2 * sh)
          ..lineTo(cx, cy + 10 * sh)
          ..lineTo(cx - 9 * sw, cy + 5 * sh)
          ..close();
