  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        // Right column — top image taller (has close button), bottom shorter
        Expanded(
          child: Column(
            children: [
              // Top right — taller, with ✕ button
              Expanded(
                flex: 52,
                child: Stack(
                  children: [
...
class _LivingRoomPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Large plant (left)
    _drawPlantBlob(canvas, Offset(w * 0.08, h * 0.62), w * 0.12, h * 0.35,
    ...
    // Round mirror
    canvas.drawCircle(Offset(w * 0.62, h * 0.22), w * 0.12,
    ...
    // Right plants
    _drawPlantBlob(canvas, Offset(w * 0.85, h * 0.60), w * 0.1, h * 0.30,
        const Color(0xFF3A6020));

    // White globe lamp
    canvas.drawCircle(
        Offset(w * 0.72, h * 0.46), w * 0.07, Paint()..color = Colors.white);
  }

  void _drawPlantBlob(Canvas canvas, Offset base, double w, double h, Color color) {
    final paint = Paint()..color = color;
    canvas.drawOval(Rect.fromLTWH(base.dx - w / 2, base.dy - h, w, h), paint);
    canvas.drawOval(
        Rect.fromLTWH(base.dx - w * 0.7, base.dy - h * 0.7, w * 0.6, h * 0.5), paint);
    canvas.drawOval(
        Rect.fromLTWH(base.dx + w * 0.1, base.dy - h * 0.6, w * 0.5, h * 0.4), paint);
  }
...
class _ExteriorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Sky — dusk grey-blue
    canvas.drawRect(Rect.fromLTWH(0, 0, w, h * 0.45),
        Paint()..color = const Color(0xFF7888A0));

    // Dark trees silhouette
    canvas.drawRect(Rect.fromLTWH(0, h * 0.1, w, h * 0.36),
        Paint()..color = const Color(0xFF2A3828));

    // House facade — stone/concrete
    canvas.drawRect(Rect.fromLTWH(w * 0.05, h * 0.20, w * 0.9, h * 0.45),
        Paint()..color = const Color(0xFF8A8A7A));

    // Roof overhang
    canvas.drawRect(Rect.fromLTWH(w * 0.02, h * 0.18, w * 0.96, h * 0.05),
        Paint()..color = const Color(0xFF3A3A30));

    // Glowing windows
    final glowPaint = Paint()..color = const Color(0xFFE8B840);
    canvas.drawRect(Rect.fromLTWH(w * 0.1, h * 0.24, w * 0.35, h * 0.18), glowPaint);
    canvas.drawRect(Rect.fromLTWH(w * 0.55, h * 0.24, w * 0.35, h * 0.18), glowPaint);
    // Window glow bloom
    final bloomPaint = Paint()
      ..color = const Color(0xFFE8B840).withOpacity(0.3)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 12);
    canvas.drawRect(Rect.fromLTWH(w * 0.05, h * 0.20, w * 0.9, h * 0.25), bloomPaint);
    ...
    for (final pos in [
      [0.28, 0.78], [0.42, 0.70], [0.58, 0.66], [0.65, 0.73], [0.48, 0.82]
    ]) {
    ...
    for (final pos in [[0.12, 0.68], [0.22, 0.70], [0.75, 0.68], [0.85, 0.70]]) {
...
class _ModernLivingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Floor — warm beige
    canvas.drawRect(Rect.fromLTWH(0, h * 0.70, w, h * 0.30),
    ...
    final skylinePaint = Paint()..color = const Color(0xFF0A1828).withOpacity(0.7);
    final skyPath = Path()
      ..moveTo(0, h * 0.45)
    ...
    final lampPaint = Paint()
      ...
      ..strokeWidth = 2;
    canvas.drawLine(Offset(w * 0.80, h * 0.70), Offset(w * 0.80, h * 0.40), lampPaint);
    canvas.drawOval(
...
class _GreenBedroomPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(Rect.fromLTWH(0, 0, w * 0.18, h * 0.75),
    ...
    for (int i = 0; i < 3; i++) {
      canvas.drawLine(
          Offset(bulbPositions[i], 0), Offset(bulbPositions[i], bulbHeights[i]), cordPaint);
      // Bulb
      canvas.drawCircle(
